...
class Tree
  def initialize(array, node_type=Node)
    @root = nil
    build_tree(merge_sort(array.uniq), node_type)
    # p merge_sort(array.uniq)
  end

  def build_tree(array, root_call = true)

    f = lambda {|x| return (x.is_a?(Node) or x.is_a?(String)) ? x : x.data}

    if array.length <= 1
      return Node.new(f.call(array[0]))
    end
    ...
    if middle > 0
      left_node = build_tree(array[0, middle].map(&f), false)
    else
      left_node = nil
    end

    if (array.length - 1) - middle > 0
      right_node = build_tree(array[middle + 1, array.length - 1].map(&f), false)
    else
      right_node = nil
    end

    tree = Node.new(f.call(array[middle]), left_node, right_node)
